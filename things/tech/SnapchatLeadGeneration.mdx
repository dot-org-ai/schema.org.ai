---
$id: https://tech.org.ai/SnapchatLeadGeneration
$context: https://schema.org.ai
$type: Technology
name: Snapchat Lead Generation
description: Communication platform for business automation
url: https://snapchatleadgeneration.com
applicationCategory: Communication Software
provider:
  $type: Organization
  name: Snapchat Lead Generation
  url: https://snapchatleadgeneration.com
zapierSlug: snapchat-lead-generation
digital: 1.0
---

[Thing](../Thing.mdx) > [CreativeWork](../CreativeWork.mdx) > [SoftwareApplication](../SoftwareApplication.mdx) > [Technology](../Technology.mdx)

# Snapchat Lead Generation

Communication platform for business automation and workflow integration.

## Properties

| Property | Type | Description | Inherited From |
|----------|------|-------------|----------------|
| `$type` | Text | Always "Technology" | - |
| `$id` | URL | Unique identifier | Thing |
| `name` | Text | Service name | Thing |
| `description` | Text | Service description | Thing |
| `url` | URL | Service website | Thing |
| `provider` | Organization | Service provider | SoftwareApplication |
| `applicationCategory` | Text | Software category | SoftwareApplication |
| `zapierSlug` | Text | Zapier identifier | Technology |
| `events` | Event[] | Available events | Technology |
| `actions` | Action[] | Available actions | Technology |

## Available Events

- Lead.created

## Available Actions

- Create record
- Update record
- Find record
- Delete record

## Usage in Business-as-Code

```typescript
import { $ } from 'sdk.do'

const snapchatleadgeneration = {
  $type: 'Technology',
  name: 'Snapchat Lead Generation',
  url: 'https://snapchatleadgeneration.com'
}

await $.Technology.create(snapchatleadgeneration)
```

## Resources

- [Snapchat Lead Generation](https://snapchatleadgeneration.com)
- [Zapier Integration](https://zapier.com/apps/snapchat-lead-generation/integrations)

## License

This documentation is based on Schema.org vocabulary, which is licensed under Creative Commons Attribution-ShareAlike License (version 3.0).
